...
def aligner(lines1, lines2):
    ...
    for fromdata, todata, flag in diffs:
        try:
            ...
            if todata[1].startswith('\0+'):
                fromlist.append(None)
            else:
                fromlist.append(fromdata[1])
            if fromdata[1].startswith('\0-'):
                tolist.append(None)
            else:
...
class CustomMainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.left_editor = QsciScintilla()
        self.configureEditor(self.left_editor)

        self.right_editor = QsciScintilla()
        self.configureEditor(self.right_editor)
        ...
        self.left_editor.verticalScrollBar().valueChanged.connect( self.right_editor.verticalScrollBar().setValue)
    ...
    def updateDiffView(self):
        ...
        left, right, flags = aligner(lines1, lines2)
        self.updateText(self.left_editor, left)
        self.updateText(self.right_editor, right)
    ...
    def updateText(self, editor, text):
        ...
        for linenumber, l in enumerate(text):
            ...
            if l is None:
                ...
                if annotation is None:
                    ...
                    annotation+="\n<"
                skipped_lines+=1
                editor.setMarginText(idx, "~", marginTextStyle)
            else:
                if annotation is not None:
                    editor.annotate(idx-1, annotation, 0)
                    annotation=None
                if '\0' in l or '\1' in l:
                    editor.append(l.replace('\0+', '').replace('\0-', '').replace('\m', '').replace('\0^', '').replace('\1', ''))
                    editor.markerAdd(idx, QsciScintilla.Circle)
                    editor.setMarginText(idx, l[1], marginTextStyle)
                    editor.fillIndicatorRange(idx, l.find('\0'), idx, l.rfind("\1"), 0)
                else:
                    editor.append(l)
